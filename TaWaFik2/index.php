...
<title>Anonymous</title>
   <center>
   <br>
  <h5> ==> TaWaFik 2 Project coded by Khalid <== </h5>
...
#
fwrite($fp, '$boxs = "'.$boxs . '";'."\n"."\n");
fwrite($fp, '$possibilities = ');

fwrite($fp, "count(\$number)" );
foreach ( range(1, $boxs-1 ) as $ny1) {
	fwrite($fp,"*" . "((count(\$number)) - " . $ny1 .")");
}
fwrite($fp,";");

#$possibilities = count($number) * ((count($number)-1)) *  ((count($number)-2));
#echo (count($number));
#foreach ( range(1, $boxs-1 ) as $ny1) {
#	echo ("*" . (count($number)-$ny1));

}
#

fwrite($fp, "\n".'	echo (\'Number of possibilities is \' . $possibilities . "<br>");'."\n");
...
fwrite($fp, str_repeat($space, $n1).'if (');
foreach ( range(1, $boxs-1 ) as $nx1 ) {
  foreach ( range($nx1+1, $boxs ) as $nx2 ) {
   fwrite($fp,' $n'.$nx1.' == $n'.$nx2);
       if ($nx1 == $boxs-1){
        break;
		}
   fwrite($fp,' or');
