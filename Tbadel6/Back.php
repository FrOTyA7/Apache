...
<title>Anonymous</title>
   <center>
   <br>
  <h5> ==> Tbadel 6 Project coded by Khalid <== </h5>
...
fwrite($fp, '<?php'."\n");
fwrite($fp, "\n");
fwrite($fp,"$fptxt = fopen('Download.txt', 'w');");
#
fwrite($fp, '$el0 = "' . $el0 . '";'."\n"."\n");
fwrite($fp, '$number = explode(\' \', $el0);     // Extend'."\n");
fwrite($fp, '$number = array_unique($number);  // Remove duplicates '."\n");
...
fwrite($fp,"fwrite($fptxt, str_repeat($space, ($n1)+2) . 'echo (\"$number[$n1]\"'););");   #
foreach ( range(2, $boxs ) as $n2 ) {
fwrite($fp,'fwrite($fptxt,.\"$number[$n\' . $n2 . \']\" );');   #
}
fwrite($fp, ');');
foreach ( range($boxs-1,0 ) as $n3 ) {
	fwrite($fp, "\n".str_repeat($space, ($n3)+2) . '}');
